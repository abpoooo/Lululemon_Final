import React, {useState} from 'react';
import './PaymentBackend.scss';
import CheckCircleOutlineIcon from '@mui/icons-material/CheckCircleOutline';
import LocalShippingOutlinedIcon from '@mui/icons-material/LocalShippingOutlined';
import AddIcon from '@mui/icons-material/Add';
import PaymentBackendNestedList from "./PaymentBackendNestedList";
import PaymentBackendOrderSummary from "./PaymentBackendOrderSummary";
import {PaymentBackendBillingBox} from "./PaymentBackendBillingBox";


export const PaymentBackend = () => {

    let info = JSON.parse(localStorage.getItem('info'))
    const [hideBillingAddress, showBillingAddress] = useState(true)

    return (
        <>
            <div className='paymentBackendLogo'>
                <img
                    src="data:image/png;base64,"
                    alt=""/>
            </div>

            <h2>Checkout</h2>

            <div className='paymentBackendCheckoutRow'>
                <div className='paymentBackendCheckout'>

                    <div className='informationTab'>
                        <div className='rowTab'>
                            <h3>
                                <CheckCircleOutlineIcon/>
                                Notifications to
                            </h3>

                            <div className='matchMarginWithH3'>
                                <div className='pBold'>
                                    Email
                                </div>

                                {info.email}
                            </div>

                            <a href="javascript:history.go(-1)" className='pEdit'>
                                <p>Edit</p>
                            </a>
                        </div>

                        <hr/>

                        <div className='rowTab'>
                            <h3>
                                <CheckCircleOutlineIcon/>
                                Sending to
                            </h3>

                            <div className='matchMarginWithH3'>
                                <div className='pBold'>
                                    Address
                                </div>

                                {info.firstName + ' ' + info.lastName} <br/>
                                {info.Address} <br/>
                                {info.City + ', ' + info.Province + ' ' + info.PostalCode} <br/>
                                {info.PhoneNumber}
                            </div>

                            <a href="javascript:history.go(-1)" className='pEdit'>
                                <p>Edit</p>
                            </a>
                        </div>

                        <hr/>

                        <div className='rowTab'>
                            <h3>
                                <CheckCircleOutlineIcon/>
                                Estimated delivery
                            </h3>

                            <div className='matchMarginWithH3'>
                                <div className='pBold'>
                                    <LocalShippingOutlinedIcon/>
                                </div>
                                Thu, Oct 27 (FREE)
                            </div>

                            <a href="javascript:history.go(-1)" className='pEdit'>
                                <p>Edit</p>
                            </a>
                        </div>
                        <hr/>
                    </div>

                    <div className='paymentMethodTab'>
                        <h3>Payment method</h3>
                        <p><AddIcon/>Add a lululemon gift card</p>
                        <div className='paymentTypes'>
                            <PaymentBackendNestedList/>
                        </div>

                            <h4>Billing Address</h4>
                        <div className='paymentBillingAddress'>
                            <div className='billingAddress' onClick={() => showBillingAddress(!hideBillingAddress)}>
                                <PaymentBackendBillingBox isTrue={hideBillingAddress} />
                            </div>
                            <p id='pBill'>Same as shipping address</p>
                        </div>

                        {info.firstName + ' ' + info.lastName} <br/>
                        {info.Address} <br/>
                        {info.City + ', ' + info.Province + ' ' + info.PostalCode} <br/>
                        {info.PhoneNumber}
                    </div>
                </div>

                <PaymentBackendOrderSummary/>

            </div>

            {/*iPhone 12 Pro*/}
            <div className='paymentBackendCheckoutColumn'>

                <PaymentBackendOrderSummary/>

                <div className='paymentBackendCheckout'>

                    <div className='informationTab'>
                        <div className='rowTab'>
                            <h3>
                                <CheckCircleOutlineIcon/>
                                Notifications to
                            </h3>

                            <a href="javascript:history.go(-1)" className='pEdit'>
                                <p>Edit</p>
                            </a>

                            <div className='matchMarginWithH3'>
                                {info.email}
                            </div>
                        </div>

                        <hr/>

                        <div className='rowTab'>
                            <h3>
                                <CheckCircleOutlineIcon/>
                                Sending to
                            </h3>

                            <a href="javascript:history.go(-1)" className='pEdit'>
                                <p>Edit</p>
                            </a>

                            <div className='matchMarginWithH3'>
                                {info.firstName + ' ' + info.lastName} <br/>
                                {info.Address} <br/>
                                {info.City + ', ' + info.Province + ' ' + info.PostalCode} <br/>
                                {info.PhoneNumber}
                            </div>


                        </div>

                        <hr/>

                        <div className='rowTab'>

                                <h3>
                                    <CheckCircleOutlineIcon/>
                                    Estimated delivery
                                </h3>

                                <a href="javascript:history.go(-1)" className='pEdit'>
                                    <p>Edit</p>
                                </a>

                            <div className='matchMarginWithH3'>
                                Thu, Oct 27 (FREE)
                            </div>
                        </div>
                        <hr/>
                    </div>

                    <div className='paymentMethodTab'>
                        <h3>Payment method</h3>
                        <p><AddIcon/>Add a lululemon gift card</p>
                        <div className='paymentTypes'>
                            <PaymentBackendNestedList/>
                        </div>

                        <h4>Billing Address</h4>
                        <div className='paymentBillingAddress'>
                            <div className='billingAddress' onClick={() => showBillingAddress(!hideBillingAddress)}>
                                <PaymentBackendBillingBox isTrue={hideBillingAddress} />
                            </div>
                            <p id='pBill'>Same as shipping address</p>
                        </div>

                        {info.firstName + ' ' + info.lastName} <br/>
                        {info.Address} <br/>
                        {info.City + ', ' + info.Province + ' ' + info.PostalCode} <br/>
                        {info.PhoneNumber}
                    </div>
                </div>
            </div>

            <div className='paymentBackendFooterColumn'>
                <div className='paymentBackendFooterTitle'>
                    <p>Contact Us</p>
                    <p>1.877.263.9300</p>
                    <hr/>
                </div>

                <div className='paymentBackendFooterSubtitle'>
                    <p>Shipping Policy</p>
                    <p>Privacy Policy (Last Updated: 9/28/22)</p>
                    <p>Terms of Use (Last Updated: 10/5/22)</p>
                    <p>Accessibility Statement</p>
                    <p>© lululemon athletica 1818 Cornwall Ave, Vancouver BC V6J 1C7</p>
                </div>
            </div>
            {/*iPhone 12 Pro*/}

            <div className='paymentBackendFooter'>
                <div className='paymentBackendFooterTitle'>
                    <p>Contact Us</p>
                    <p>Live Chat</p>
                    <p>1.877.263.9300</p>
                    <hr/>
                </div>

                <div className='paymentBackendFooterSubtitle'>
                    <p>Shipping Policy</p>
                    <p>Privacy Policy (Last Updated: 9/28/22)</p>
                    <p>Terms of Use (Last Updated: 10/5/22)</p>
                    <p>Accessibility Statement</p>
                    <p>© lululemon athletica 1818 Cornwall Ave, Vancouver BC V6J 1C7</p>
                </div>
            </div>

        </>

    )
}